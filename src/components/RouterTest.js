...
function Detail({ match, history, location }) {
  ...
  return (
    <div>
      <h3>Detail</h3>
      {match.params.name}
      <button onClick={history.goBack}>后退</button> {/* 类似vuerouter的 this.$router.back() */}
    </div>
  );
}
...
// 路由守卫：定义一个PrivateRoute组件   为其扩展一个用户状态检查功能
  ...
  render() {
    return (
      <BrowserRouter>
        ...
        {/* 路由配置  react-router匹配不是独占的  vue-router只要一个匹配到后边的路由就不会再匹配了 */}
        <Switch>
          ...
          <Route component={() => <h3>页面不存在</h3>} />  {/* 404 */}
        </Switch>
      </BrowserRouter>
    );
  }
